...
async function dispatchBatch(admin: any, actor: any, input: any) {
  ...
  for (const candidate of queued || []) {
    ...
    const selectedPhone = normalizePhone(candidate.phone_number) || phones[0]?.phone || "";
    const selectedSlot = candidate.phone_slot || phones.find((row) => row.phone === selectedPhone)?.slot || phones[0]?.slot || "";
    if (!compliance.allowed || !selectedPhone) {
      const reason = selectedPhone ? compliance.reasons.join(", ") : "No callable phone number";
  ...
  return {
    ...
    message: launched.length ? `${launched.length} broadcast call(s) started.` : "No callable accounts were started in this cycle.",
  };
}
...
async function campaignControl(admin: any, actor: any, input: any) {
  ...
  return { success: true, campaign: data, canceled };
}
