...
async function handleAnswer(req: Request, params: URLSearchParams, admin: any, callRow: any, campaign: any) {
  ...
  if (machineLike) {
    if (campaign.leave_voicemail && firstText(campaign.voicemail_message_text)) {
      ...
      return xml(`<Say voice="alice">${xmlEscape(campaign.voicemail_message_text)}</Say><Hangup/>`);
    }
  ...
  if (campaign.connect_mode === "auto_transfer") {
    ...
    return xml(`<Say voice="alice">${xmlEscape(message)}</Say>${dialQueueTwiml(req, campaign, callRow)}`);
  }
  ...
  return xml(`<Gather input="dtmf" numDigits="1" timeout="8" actionOnEmptyResult="true" action="${xmlEscape(gatherAction)}" method="POST"><Say voice="alice">${xmlEscape(message)}</Say></Gather><Say voice="alice">We did not receive a response. Goodbye.</Say><Hangup/>`);
}
...
async function handleGather(req: Request, params: URLSearchParams, admin: any, callRow: any, campaign: any) {
  ...
  if (digits === "1") {
    ...
    return xml(`<Say voice="alice">Please hold while we connect you.</Say>${dialQueueTwiml(req, campaign, callRow)}`);
  }
