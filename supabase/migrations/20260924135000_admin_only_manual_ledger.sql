create or replace function public.cpcm_post_manual_ledger_payment(target_account_id uuid,payment_amount numeric,payment_type_text text,payment_method_text text,payment_status_text text,payment_date_value date,receipt_text text,processor_fee_value numeric default 0,notes_text text default null,actor_email text default null) returns jsonb language plpgsql security invoker set search_path=public as $$
declare a public.accounts%rowtype; amt numeric; before_bal numeric; after_bal numeric; ledger_id uuid; effect numeric:=0; actor text:=lower(coalesce(auth.jwt()->>'email',''));
begin
 ...
 select id into ledger_id from public.payments_ledger where receipt_number=receipt_text limit 1;
 if ledger_id is not null then return jsonb_build_object('ok',true,'already_posted',true,'ledger_id',ledger_id,'account_id',target_account_id); end if;
 amt:=greatest(coalesce(payment_amount,0),0); if amt<=0 then raise exception 'Payment amount must be greater than zero'; end if;
 before_bal:=greatest(coalesce(a.current_balance,0),0);
