...
async function completeCall(admin: any, actor: any, input: any) {
  ...
  const { data: campaign } = await admin.from("dialer_campaigns").select("max_attempts").eq("id", call.campaign_id).maybeSingle();
  const attemptCount = Number(campaignAccount?.attempt_count || 0) + 1;
  const retryable = ["no answer", "busy", "left voicemail"].includes(disposition.toLowerCase());
  const shouldRetry = retryable && attemptCount < Number(campaign?.max_attempts || 1);
  const campaignAccountPatch: Record<string, unknown> = {
    status: shouldRetry ? "queued" : "completed",
    attempt_count: attemptCount,
    last_result: disposition,
    last_error: null,
    updated_at: new Date().toISOString(),
  };
  if (shouldRetry) campaignAccountPatch.sort_order = Date.now();
  await admin.from("dialer_campaign_accounts").update(campaignAccountPatch).eq("id", call.campaign_account_id);

  const accountPatch: Record<string, unknown> = {
    status: disposition,
    disposition,
    last_contact_number: call.phone_number,
    updated_at: new Date().toISOString(),
  };
  if (disposition.toLowerCase() === "dnc") accountPatch.do_not_call = true;
  if (["bad number", "wrong number"].includes(disposition.toLowerCase())) accountPatch.wrong_number_flag = true;
  const { error: accountUpdateError } = await admin.from("accounts").update(accountPatch).eq("id", call.account_id);
  if (accountUpdateError) {
    await admin.from("accounts").update({ status: disposition, disposition, updated_at: new Date().toISOString() }).eq("id", call.account_id);
  }
  await insertActivity(
    admin,
    call.account_id,
    "Call Result",
    `RingCentral power dialer result: ${disposition}${notes ? ` — ${notes}` : ""}.`,
    call.employee_email,
    call.phone_number,
  );
